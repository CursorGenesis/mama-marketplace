...
export default function AgentsPage() {
  ...
  const faq = [
    ...
    },
    {
      ...
      aRu: 'Вы можете пригласить поставщика — он получит бесплатный месяц. А вы будете получать 1-2% от всех заказов через платформу.',
      qKg: 'Жеткирүүчү платформада болбосочу?',
      aKg: 'Жеткирүүчүнү чакырсаңыз болот — ал акысыз ай алат. Ал эми сиз платформа аркылуу бардык заказдардан 1-2% аласыз.',
    },
    {
      ...
      <div className="bg-gradient-to-br from-slate-800 via-slate-900 to-green-900 py-14 sm:py-20">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <p className="text-slate-300 text-lg max-w-2xl mx-auto mb-8">
            {isRu
              ? 'Подключай магазины, оформляй заказы через приложение вместо бумажки — и получай 1-2% от каждого заказа.'
              : 'Дүкөндөрдү туташтырыңыз, заказдарды кагаздын ордуна тиркеме аркылуу тариздеңиз — жана ар бир заказдан 1-2% алыңыз.'}
          </p>
          <Link
      ...
      <div className="max-w-4xl mx-auto px-4">
          ...
          {[
            {
              ...
              titleRu: '1-2% от каждого заказа', titleKg: 'Ар бир заказдан 1-2%',
              descRu: 'Магазин заказывает через платформу — вы получаете 2% (мелкие поставщики) или 1% (крупные). Каждый месяц пока вы агент.',
              descKg: 'Дүкөн платформа аркылуу заказ кылат — 2% (кичине жеткирүүчүлөр) же 1% (ири) аласыз. Агент болуп жатканда ар бир ай.',
            },
            {
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 mb-12">
          ...
          <div className="flex gap-2 mb-4">
            <button onClick={() => setPercent(2)}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${percent === 2 ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-600'}`}>
              2% — {isRu ? 'мелкие поставщики' : 'кичине жеткирүүчүлөр'}
            </button>
            <button onClick={() => setPercent(1)}
              className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-colors ${percent === 1 ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-600'}`}>
              1% — {isRu ? 'крупные поставщики' : 'ири жеткирүүчүлөр'}
            </button>
          </div>
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
            {[
              ...
              { step: 4, emoji: '💰', titleRu: 'Получайте 1-2%', titleKg: '1-2% алыңыз', descRu: 'С каждого заказа ваших магазинов пока вы работаете агентом', descKg: 'Агент болуп иштеп жатканда дүкөндөрүңүздүн ар бир заказынан' },
            ].map(s => (
